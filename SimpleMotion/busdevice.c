...
#endif
    if(strncmp(devicename,"MBEDSERIAL",9) == 0 ) //use mbed lib
	{
    	//this choice has only one port, no need to init it
    	BusDevice[handle].comPort=1;
		BusDevice[handle].bdType=BD_MBEDSERIAL;
        BusDevice[handle].txBufferUsed=0;
	}
...
smbool smBDClose( const smbusdevicehandle handle )
{
	...
	if( BusDevice[handle].bdType==BD_MBEDSERIAL )
	{
...
smbool smBDTransmit(const smbusdevicehandle handle)
{
    ...
    if( BusDevice[handle].bdType==BD_MBEDSERIAL )
    {
        if( SMPortWrite(BusDevice[handle].txBuffer, BusDevice[handle].txBufferUsed)==BusDevice[handle].txBufferUsed)
        {
            BusDevice[handle].txBufferUsed=0;
            return smtrue;
        }
